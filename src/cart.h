...
class Cart
{
  public:
    ...
    static uint8_t writeByte(uint8_t data) /*__attribute__((always_inline))*/; // write a single byte to flash memory.
    ...
    static void Cart::displayPrefetch(uint24_t address, uint8_t* target, uint16_t len, bool clear);
    ...
    static void seekCommand(uint8_t command, uint24_t address)/* __attribute__((always_inline))*/;// Write command and selects flash memory address. Required by any read or write command

    static void seekData(uint24_t address)/* __attribute__((always_inline))*/; // selects flashaddress of program data area for reading and starts the first read
    ...
    static uint8_t readEnd()/* __attribute__((always_inline))*/;
